...
const METRICS = [
  { label: 'Total Tasks',  value: '248', icon: '📋', iconColor: '#2c2abc', sub: '+12% vs last week',     subColor: '#1d9681' },
  { label: 'Completed',    value: '186', icon: '✅', iconColor: '#1d9681', sub: '75% completion rate',   subColor: '#464554' },
  { label: 'In Progress',  value: '42',  icon: '🕐', iconColor: '#0aa5e9', sub: '17% of total workload', subColor: '#464554' },
  { label: 'Overdue',      value: '20',  icon: '⚠️', iconColor: '#ba1a1a', sub: '8% needs attention',    subColor: '#ba1a1a' },
];
...
export default function DashboardMonth({ navigation }) {
  return (
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.topBlock}>
          ...
          <View style={styles.tabBar}>
            {TABS.map((tab) => {
              const isActive = tab.label === 'This Month';
              return (
                <TouchableOpacity
                  key={tab.label}
        ...
        >
          <Text style={styles.ctaButtonText}>Details this month's tasks →</Text>
        </TouchableOpacity>

        <View style={styles.chartWrapper}>
          <LineChart variant="month" onViewDetails={() => navigation.navigate('TaskStatus')} />
        </View>
